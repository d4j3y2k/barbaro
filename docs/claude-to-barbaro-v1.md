...
parent chain. UUID-bearing rows physically after that pointer may extend the selection along a unique
semantic continuation from its leaf. Sibling assistant rows may continue together when they share a
response identity; sibling tool results require distinct result IDs, exactly one accepted call and
one result for each ID, and matching source-call UUIDs when present. These checks admit a late parallel
tool group, terminal response and next human prompt without resurrecting descendants the pointer had
already rejected. `logicalParentUuid` carries the chain across compaction. UUID-less sidecars and UUID-bearing branches whose entire subtree contains
only attachment records are transparent; inline attachments that lead to a user, assistant, or system
row remain part of the continuation. Competing response groups, ambiguous call/result identities,
duplicate post-pointer UUIDs, a rewind from an older ancestor, a cycle, or
...
| Finality, two claims | `final` means the trailing turn may close; `sourceFinal` means the file will not change again. A Stop hook knows the first, not the second, so it passes `final: true, sourceFinal: false`. A turn closed in the stream may publish: a successor prompt closes the turn before it, and the provider's own `system/turn_duration` record — written once per turn, after the stop hooks, after every row of the final response — closes a terminal turn that has no background launch outstanding. The asynchronous Stop ingest waits up to four seconds for that record, including on the first turn with no prior closing-record observation. A missing close is reported as `trailing_turn_close_timeout`; it never authorizes publication. A turn whose background launch can still report back is withheld without waiting for a close. A subsequent prompt can close a withheld turn, and SessionEnd/offline ingestion can assert `sourceFinal`. A pointerless fork likewise waits for `sourceFinal`. Hook output counters include every retry's publications; snapshot diagnostics describe the final pass. |
...
| Active branch | Resolved from that snapshot: `last-prompt.leafUuid` walks back through `parentUuid ?? logicalParentUuid`. A semantic continuation written physically after the selected pointer can extend that ancestry, including sibling rows sharing an assistant response identity and uniquely paired sibling results for its accepted calls. A UUID-bearing side branch is transparent only when its entire subtree is attachment records (including any PreToolUse hook's `additionalContext` sidecars); attachments that lead to semantic rows remain in the continuation. Competing response groups, ambiguous calls/results, duplicate post-pointer UUIDs, rewinds, cycles and disconnected semantic continuations withhold the snapshot, even at SessionEnd. Descendants already present before the pointer remain excluded because they may be the branch it rejected. A trace with no `last-prompt` treats every record as on-branch rather than guessing. |
